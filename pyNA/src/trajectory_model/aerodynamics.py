...
class Aerodynamics(om.MetaModelStructuredComp):
    """
    ...
    * ``outputs['c_d']``:                aircraft drag coefficient [-]
    ...
    def setup(self):
        ...
        self.add_input('alpha', val=np.ones(nn), units='deg', training_data=aircraft.aero['alpha'])
        self.add_input('theta_flaps', val=np.ones(nn), units='deg', training_data=aircraft.aero['theta_flaps'])
        self.add_input('theta_slats', val=np.ones(nn), units='deg', training_data=aircraft.aero['theta_slats'])

        self.add_output('c_l', val=np.ones(nn), units=None, training_data=aircraft.aero['c_l'])
        self.add_output('c_l_max', val=np.ones(nn), units=None, training_data=aircraft.aero['c_l_max'])
        self.add_output('c_d', val=np.ones(nn), units=None, training_data=aircraft.aero['c_d'])