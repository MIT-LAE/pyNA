function jet_mixing_source!(spl, pyna_ip, settings, ac, f, M_0, c_0, theta, TS, V_j_star, rho_j_star, A_j_star, Tt_j_star)
    ...
    mlog10F = pyna_ip.f_log10F_jet.(theta*ones(settings["n_frequency_bands"],), Tt_j_star*ones(settings["n_frequency_bands"],), log10Vja0*ones(settings["n_frequency_bands"],), log10St)
    ...
    if settings["core_jet_suppression"] && settings["case_name"] in ["nasa_stca_standard", "stca_enginedesign_standard"] && TS > 0.8
...
# Function 
function jet_shock_source!(spl, pyna_ip, settings, ac, f, M_0, c_0, theta, TS, V_j_star, M_j, A_j_star, Tt_j_star)
