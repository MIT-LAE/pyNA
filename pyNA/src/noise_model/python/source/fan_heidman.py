...
def inlet_broadband(settings: dict, tables: Tables, theta: float, M_tip: float, tsqem: float, fan_M_d: float, fan_RSS: float) -> float:
    ...
    M_tip : float
        relative (i.e., helical) tip Mach number [-]
    tsqem : flaot
        broadband temperature-flow power base term [-]
    fan_M_d : float
        fan rotor relative tip Mach number at design [-]
    fan_RSS : float
    ...
    if settings['fan_BB_method'] == 'original':
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 0.9:
                f1ib = 58.5 + 20 * np.log10(fan_M_d)
            else:
                f1ib = 58.5 + 20 * np.log10(fan_M_d) - 20 * np.log10(M_tip / 0.9)

        # Rotor-stator correction term (F2 of Eqn 4, Figure 6B):
        if not settings['fan_id']:
            f2ib = -5 * np.log10(fan_RSS / 300)  # If no distortion
        else:
            if fan_RSS <= 100:
                f2ib = -5 * np.log10(fan_RSS / 300)
            else:
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 0.9:
                f1ib = 55.5 + 20 * np.log10(fan_M_d)
            else:
                f1ib = 55.5 + 20 * np.log10(fan_M_d) - 20 * np.log10(M_tip / 0.9)
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 0.9:
                ...
                f1ib = 58.5 + 20 * np.log10(fan_M_d) - 50 * np.log10(M_tip / 0.9)
        ...
        if fan_M_d <= 1:
            ...
            if M_tip < 0.72:
                f1ib = 34 + 20 * np.log10(fan_M_d / 1.245)
            else:
                f1ib = 34 - 43 * (M_tip - 0.72) + 20 * np.log10(fan_M_d/ 1.245)
    ...
    f3ib = np.interp(theta, tables.source.fan.f3ib_theta, tables.source.fan.f3ib_data)
...
def discharge_broadband(settings: dict, tables: Tables, theta: float, M_tip: float, tsqem: float, fan_M_d: float, fan_RSS: float) -> float:
    """
    ...
    if settings['fan_BB_method'] == 'original':
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 1:
                f1db = 60 + 20 * np.log10(fan_M_d)
            else:
                f1db = 60 + 20 * np.log10(fan_M_d) - 20 * np.log10(M_tip / 1)

        # Rotor-stator correction term (F2 of Eqn 4, Figure 6B):
        if not settings['fan_id']:
            f2db = -5 * np.log10(fan_RSS / 300)  # If no distortion
        else:
            if fan_RSS <= 100:
                f2db = -5 * np.log10(fan_RSS / 300)
            else:
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 1:
                f1db = 58 + 20 * np.log10(fan_M_d)
            else:
                f1db = 58 + 20 * np.log10(fan_M_d) - 20 * np.log10(M_tip / 1)
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 1:
                f1db = 63 + 20 * np.log10(fan_M_d)
            else:
                f1db = 63 + 20 * np.log10(fan_M_d) - 30 * np.log10(M_tip / 1)
        ...
        if fan_M_d <= 1:
            ...
            # If M_tip < 0.65 Then
            #   f1dbBkrejsa = 34 + 20 * np.log10(fan_M_d / 1.245)
            # Else
            f1db = 34 - 17 * (M_tip - 0.65) + 20 * np.log10(fan_M_d / 1.245)
    ...
    f3db = np.interp(theta, tables.source.fan.f3db_theta, tables.source.fan.f3db_data)
...
def inlet_tones(settings: dict, tables: Tables, theta: float, M_tip: float, tsqem: float, fan_M_d: float, fan_RSS: float) -> float:
    ...
    M_tip : float
        relative (i.e., helical) tip Mach number [-]
    tsqem : float
        tone temperature-flow power base term [-]
    fan_M_d : float
        fan rotor relative tip Mach number at design [-]
    fan_RSS : float
        fan rotor-stator spacing [%]
    ...
    if settings['fan_RS_method'] == 'original':
        ...
        if fan_M_d <= 1:
            if M_tip <= 0.72:
                ...
                f1ti_a = 60.5 + 50 * np.log10(M_tip / 0.72)
                f1ti_b = 59.5 + 80 * np.log10(1. / M_tip)
                if f1ti_a < f1ti_b:
                    f1ti = f1ti_a
                else:
                    f1ti = f1ti_b
            ...
            if M_tip <= 0.72:
                f1ti = 60.5 + 20 * np.log10(fan_M_d)
            else:
                f1ti_a = 60.5 + 20 * np.log10(fan_M_d) + 50 * np.log10(M_tip / 0.72)
                # Note the 1975 version of NASA TMX-71763 writes it this way:
                # f1ti_b = 59.5 + 20 * np.log10(fan_M_d) + 80 * np.log10(fan_M_d / M_tip)
                # But the 1979 version of NASA TMX-71763 writes it this way:
                f1ti_b = 59.5 + 80 * np.log10(fan_M_d / M_tip)
                if f1ti_a < f1ti_b:
                    f1ti = f1ti_a
                else:
                    f1ti = f1ti_b
        ...
        if not settings['fan_id']:
            f2ti = -10 * np.log10(fan_RSS / 300)  # If no distortion
        else:
            if fan_RSS < 100:
                f2ti = -10 * np.log10(fan_RSS / 300)
            else:
        ...
        if fan_M_d <= 1:
            if M_tip <= 0.72:
                ...
                f1ti_a = 54.5 + 50 * np.log10(M_tip / 0.72)
                f1ti_b = 53.5 + 80 * np.log10(1. / M_tip)
                if f1ti_a < f1ti_b:
                    f1ti = f1ti_a
                else:
                    f1ti = f1ti_b
            ...
            if M_tip <= 0.72:
                f1ti = 54.5 + 20 * np.log10(fan_M_d)
            else:
                f1ti_a = 54.5 + 20 * np.log10(fan_M_d) + 50 * np.log10(M_tip / 0.72)
                f1ti_b = 53.5 + 80 * np.log10(fan_M_d / M_tip)
                if f1ti_a < f1ti_b:
                    f1ti = f1ti_a
                else:
                    f1ti = f1ti_b
        ...
        if fan_M_d <= 1:
            if M_tip <= 0.72:
                ...
                f1ti_a = 60.5 + 50 * np.log10(M_tip / 0.72)
                f1ti_b = 64.5 + 80 * np.log10(1. / M_tip)
                if f1ti_a < f1ti_b:
                    f1ti = f1ti_a
                else:
                    f1ti = f1ti_b
            ...
            if M_tip <= 0.72:
                ...
                f1ti_a = 60.5 + 20 * np.log10(fan_M_d) + 50 * np.log10(M_tip / 0.72)
                f1ti_b = 64.5 + 80 * np.log10(fan_M_d) - 80 * np.log10(M_tip)
                if f1ti_a < f1ti_b:
                    f1ti = f1ti_a
                else:
                    f1ti = f1ti_b
        ...
        if fan_M_d <= 1:
            ...
            f1ti = 42 - 20 * M_tip + 20 * np.log10(fan_M_d / 1.245)
    ...
    f3ti = np.interp(theta, tables.source.fan.f3ti_theta, tables.source.fan.f3ti_data)
...
def discharge_tones(settings: dict, tables: Tables, theta: float, M_tip: float, tsqem: float, fan_M_d: float, fan_RSS: float) -> float:
    ...
    if settings['fan_RS_method'] == 'original':
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 1:
                f1td = 63 + 20 * np.log10(fan_M_d)
            else:
                f1td = 63 + 20 * np.log10(fan_M_d) - 20 * np.log10(M_tip / 1)

        # Rotor-stator spacing correction term (F2 of Eqn 12, Figure 12):
        if not settings['fan_id']:
            f2td = -10 * np.log10(fan_RSS / 300)  # If no distortion
        else:
            if fan_RSS < 100:
                f2td = -10 * np.log10(fan_RSS / 300)
            else:
        ...
        if fan_M_d <= 1:
            ...
            if M_tip <= 1:
                f1td = 59 + 20 * np.log10(fan_M_d)
            else:
                f1td = 59 + 20 * np.log10(fan_M_d) - 20 * np.log10(M_tip / 1)
        ...
        if fan_M_d <= 1:
            ...
            f1td = 46 - 20 * M_tip + 20 * np.log10(fan_M_d / 1.245)
    ...
    f3td = np.interp(theta, tables.source.fan.f3td_theta, tables.source.fan.f3td_data)
...
def combination_tones(settings: dict, tables: Tables, f: np.ndarray, theta: float, M_tip: float, bpf: float, tsqem: float) -> np.ndarray:
    ...
    if M_tip >= 1:
        ...
        f2ct = np.interp(theta, tables.source.fan.f2ct_theta, tables.source.fan.f2ct_data)
...
def calculate_cutoff(M_tip_tan: float, fan_B: float, fan_V: float) -> int:
    ...
    ---------
    M_tip_tan : float
        tangential (i.e., radius*omega) tip Mach number [-]
    fan_B : int
        fan blade number [-]
    fan_V : int
        fan vane number [-]
...
def calculate_harmonics(settings: dict, tables: Tables, f: np.ndarray, theta: float, tonlv_I: float, tonlv_X: float, i_cut: int, M_tip: float, bpf: float, comp: str) -> Tuple:
    ...
    tonlv_I : float
        inlet tone level [-]
    tonlv_X : float 
        discharge tone level [-]
    i_cut : int
        cut-off parameter (0/1)
    M_tip : float
        relative (i.e., helical) tip Mach number [-]
    bpf : float
        blade pass frequency
    comp : str
        fan component (fan_inlet / fan_discharge)
    ...
    for ih in np.arange(1, settings['n_harmonics'] + 1):
        ...
        if settings['fan_id'] and settings['fan_RS_method'] != 'geae':
            ...
            if settings['fan_ge_flight_cleanup'] == 'takeoff':
                if ih == 1:
                    TCS = np.interp(theta, tables.source.fan.fe_theta, tables.source.fan.fe_takeoff_1_data)
                elif ih == 2:
                    TCS = np.interp(theta, tables.source.fan.fe_theta, tables.source.fan.fe_takeoff_2_data)
                else:
                    TCS = 0.

            elif settings['fan_ge_flight_cleanup'] == 'approach':
                if ih == 1:
                    TCS = np.interp(theta, tables.source.fan.fe_theta, tables.source.fan.fe_approach_1_data)
                elif ih == 2:
                    TCS = np.interp(theta, tables.source.fan.fe_theta, tables.source.fan.fe_approach_2_data)
                else:
                    TCS = 0.
...
def fan_heidman(fan_DTt: float, fan_mdot: float, fan_N: float, M_0: float, c_0: float, rho_0: float, theta: float, f: np.ndarray, settings: dict, aircraft: Aircraft, tables: Tables, comp: str) -> np.ndarray:

    """
    
    Calculates fan noise mean-square acoustic pressure (msap) using the Heidman fan noise method (Berton's implementation).
    ...
    ### Extract the ijnputs
    M_tip_tan = (aircraft.engine.fan_d / 2) * fan_N * 2 * np.pi / 60 / c_0  # Tangential (i.e., radius*omega) tip Mach number: ! Doesn't need correction
    bpf = fan_N * aircraft.engine.fan_B / 60. / (1 - M_0 * np.cos(theta * np.pi / 180))  # Blade passing frequency, [Hz]
    ...
    if settings['fan_BB_method'] == 'kresja':
        tsqem = 10 * np.log10((fan_DTt * 1.8) ** 4 * 2.20462 * fan_mdot / (1 - M_0 * np.cos(theta * np.pi / 180)) ** 4 / settings['r_0'] ** 2 / (rho_sl ** 2 * c_sl ** 4))
    else:  # All other methods:
        tsqem = 10 * np.log10((fan_DTt * 1.8) ** 2 * 2.20462 * fan_mdot / (1 - M_0 * np.cos(theta * np.pi / 180)) ** 4 / settings['r_0'] ** 2 / (rho_sl ** 2 * c_sl ** 4))

    # Calculate individual noise components
    dcp = np.zeros(settings['n_frequency_bands'])
    if comp == 'fan_inlet':
        bblv_I = inlet_broadband(settings, tables, theta, M_tip, tsqem, aircraft.engine.fan_M_d, aircraft.engine.fan_RSS)
        tonlv_I = inlet_tones(settings, tables, theta, M_tip, tsqem, aircraft.engine.fan_M_d, aircraft.engine.fan_RSS)
        bblv_D = 0.
        tonlv_X = 0.
    elif comp == 'fan_discharge':
        bblv_I = 0.
        tonlv_I = 0.
        bblv_D = discharge_broadband(settings, tables, theta, M_tip, tsqem, aircraft.engine.fan_M_d, aircraft.engine.fan_RSS)
        tonlv_X = discharge_tones(settings, tables, theta, M_tip, tsqem, aircraft.engine.fan_M_d, aircraft.engine.fan_RSS)
    else:
    ...
    if settings['fan_BB_method'] == 'allied_signal':
        ...
        FLOGinlet = 2.445096095 * (np.log(f / bpf / 2)) ** 2
        # elif f[j] / bpf > 2:
        # FLOGinlet,exit = -10 * np.log10(np.exp(-2.0 * (np.log(f[j] / bpf / 2.0) / np.log(2.2)) ** 2))
        FLOGinlet[f/bpf > 2] = (13.97197769 * (np.log(f / bpf / 2)) ** 2)[f/bpf > 2]
        FLOGexit = FLOGinlet
        ...
        FLOGinlet = 3.4929944 * (np.log(f / bpf / 4)) ** 2
        # FLOGexit = -10 * np.log10(np.exp(-0.5 * (np.log(f[j] / bpf / 2.5) / np.log(2.2)) ** 2))
        # Which may be simplified as:
        FLOGexit = 3.4929944 * (np.log(f / bpf / 2.5)) ** 2
        ...
        # Which may be simplified as:
        FLOGinlet = 3.4929944 * (np.log(f / bpf / 2.5)) ** 2
        FLOGexit = FLOGinlet
    ...
    if settings['fan_liner_suppression']:
        if comp == 'fan_inlet':
            supp = tables.source.fan.supp_fi_interp(theta, f).reshape(settings['n_frequency_bands'],)

        elif comp == 'fan_discharge':
            supp = tables.source.fan.supp_fd_interp(theta, f).reshape(settings['n_frequency_bands'], )
