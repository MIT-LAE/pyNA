...
"""
Partials from (old) aerodynamics
--------------------------------
        ...
        partials['L', 'c_l'] = q * aircraft.af_S_w
        partials['L', 'v'] = inputs['c_l'] * rho_0 * inputs['v'] * aircraft.af_S_w
        if self.options['atmosphere_type'] == 'stratified':
            partials['L', 'rho_0'] = inputs['c_l'] * 1/2. * inputs['v']**2 * aircraft.af_S_w

        constant_LD = False
        if constant_LD:
            L_D = 6.718101501415649
            partials['D', 'c_d'] = 0.
            partials['D', 'c_l'] = q * aircraft.af_S_w * 1/L_D
            if self.options['atmosphere_type'] == 'stratified':
                partials['D', 'rho_0'] = 1/2 * inputs['v']**2 * aircraft.af_S_w * inputs['c_l']/L_D
            partials['D', 'v'] = rho_0 * inputs['v'] * aircraft.af_S_w * inputs['c_l']/L_D
        else:
            partials['D', 'c_d'] = q * aircraft.af_S_w
            partials['D', 'c_l'] = 0.
            if phase_name in {'groundroll', 'rotation', 'liftoff'}:
                if self.options['atmosphere_type'] == 'stratified':
                    partials['D', 'rho_0'] = (inputs['c_d'] + aircraft.c_d_g) * 1/2. * inputs['v']**2 * aircraft.af_S_w
                partials['D', 'v'] = (inputs['c_d'] + aircraft.c_d_g) * rho_0 * inputs['v'] * aircraft.af_S_w
            elif phase_name in {'vnrs', 'cutback'}:
                if self.options['atmosphere_type'] == 'stratified':
                    partials['D', 'rho_0'] = inputs['c_d'] * 1/2. * inputs['v']**2 * aircraft.af_S_w
                partials['D', 'v'] = inputs['c_d'] * rho_0 * inputs['v'] * aircraft.af_S_w 
