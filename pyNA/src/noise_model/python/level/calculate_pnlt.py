...
def calculate_pnlt(spl: np.ndarray, settings: dict) -> Tuple:
    ...
    noy = np.zeros((settings['n_frequency_bands'], ))
    C = np.zeros((settings['n_frequency_bands'], ))
    ...
    for i in np.arange(settings['n_frequency_bands']):
        ...
        if spl_a[i] <= spl[i]:
            noy[i] = 10 ** (m_c[i] * (spl[i] - spl_c[i]))
        elif spl_b[i] <= spl[i] <= spl_a[i]:
            noy[i] = 10 ** (m_b[i] * (spl[i] - spl_b[i]))
        elif spl_e[i] <= spl[i] <= spl_b[i]:
            noy[i] = 0.3 * 10 ** (m_e[i] * (spl[i] - spl_e[i]))
        elif spl_d[i] <= spl[i] <= spl_e[i]:
            noy[i] = 0.1 * 10 ** (m_d[i] * (spl[i] - spl_d[i]))
        else:
    ...
    n_max = np.max(noy)
    n_t = n_max + 0.15 * (np.sum(noy) - n_max)
