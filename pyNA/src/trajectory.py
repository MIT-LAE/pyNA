    ...
    def setup(self, problem: om.Problem, settings: Settings, ac: Aircraft, engine: Engine, trajectory_mode: str, objective: str) -> None:
        ...
        * ``cutback``:      a pilot-initiated thrust cut-back to a constant thrust-setting is applied
        ...
        if 'groundroll' in self.phase_name_lst:
            ...
            if settings.PKROT:
                self.phases['groundroll'].add_parameter('k_rot', targets='flight_dynamics.k_rot', units=None, lower=1.1, upper=1.8, dynamic=False, val=ac.k_rot, opt=True)
            else:
        ...
        if 'vnrs' in self.phase_name_lst:
            ...
            self.phases['vnrs'].set_time_options(initial_bounds=(10, 150), duration_bounds=(0, 500), initial_ref=100., duration_ref=100.)            
        ...
        if 'cutback' in self.phase_name_lst:
            ...
            self.phases['cutback'].set_time_options(initial_bounds=(10, 150), duration_bounds=(0, 500), initial_ref=100., duration_ref=100.)
            if trajectory_mode == 'flyover':
