...
class Noise:
    ...
    @staticmethod
    def get_input_vector_indices(settings: Settings, n_t: np.int) -> Union[Dict, Dict]:
        """
    ...
    def setup_time_series(self, problem:om.Problem, settings:Settings, ac:Aircraft, n_t:np.int, mode:str, objective:str) -> None:
        """
        ...
        if settings.language == 'python':
            ...
            idx = Noise.get_input_vector_indices(settings=settings, n_t=n_t)
            
            problem.model.add_subsystem(name='noise',
                                        subsys=make_component(julia.NoiseModel(settings, self.data, ac, n_t, idx, objective)),
                                        promotes_inputs=[],
                                        promotes_outputs=[])
    ...
    @staticmethod
    def compute_time_series(problem: om.Problem, settings: Settings, path: pd.DataFrame, engine: Engine, mode: str) -> None:
        """
        ...
        # Save the results
        if settings.save_results:
            problem.record('trajectory')

        return

    def setup_trajectory_noise(self, problem: om.Problem, settings: Settings, ac: Aircraft, n_t:np.int, objective: str) -> None:
        """
        ...
        if settings.language == 'python':
            problem.model.add_subsystem(name='noise',
                                        subsys=NoiseModel(settings=settings, data=self.data, ac=ac, n_t=n_t, mode='trajectory'), 
                                        promotes_inputs=[],
                                        promotes_outputs=[])
        
            # Create connections from trajectory group
            problem.model.connect('trajectory.c_0', 'noise.normalize_engine.c_0')
            problem.model.connect('trajectory.T_0', 'noise.normalize_engine.T_0')
            problem.model.connect('trajectory.p_0', 'noise.normalize_engine.p_0')
            problem.model.connect('trajectory.rho_0', 'noise.normalize_engine.rho_0')
            if settings.jet_mixing and settings.jet_shock == False:
                problem.model.connect('engine.V_j', 'noise.normalize_engine.V_j')
                problem.model.connect('engine.rho_j', 'noise.normalize_engine.rho_j')
                problem.model.connect('engine.A_j', 'noise.normalize_engine.A_j')
                problem.model.connect('engine.Tt_j', 'noise.normalize_engine.Tt_j')
            elif settings.jet_shock and settings.jet_mixing == False:
                problem.model.connect('engine.V_j', 'noise.normalize_engine.V_j')
                problem.model.connect('engine.A_j', 'noise.normalize_engine.A_j')
                problem.model.connect('engine.Tt_j', 'noise.normalize_engine.Tt_j')
                problem.model.connect('engine.M_j', 'noise.source.M_j')
            ...
            if settings.core:
                if settings.method_core_turb == 'GE':
                    problem.model.connect('engine.mdoti_c', 'noise.normalize_engine.mdoti_c')
                    problem.model.connect('engine.Tti_c', 'noise.normalize_engine.Tti_c')
                    problem.model.connect('engine.Ttj_c', 'noise.normalize_engine.Ttj_c')
                    problem.model.connect('engine.Pti_c', 'noise.normalize_engine.Pti_c')
                    problem.model.connect('engine.DTt_des_c', 'noise.normalize_engine.DTt_des_c')
                elif settings.methods_core_turb == 'PW':
                    problem.model.connect('engine.mdoti_c', 'noise.normalize_engine.mdoti_c')
                    problem.model.connect('engine.Tti_c', 'noise.normalize_engine.Tti_c')
                    problem.model.connect('engine.Ttj_c', 'noise.normalize_engine.Ttj_c')
                    problem.model.connect('engine.Pti_c', 'noise.normalize_engine.Pti_c')
                    problem.model.connect('engine.rho_te_c', 'noise.normalize_engine.rho_te_c')
                    problem.model.connect('engine.c_te_c', 'noise.normalize_engine.c_te_c')
                    problem.model.connect('engine.rho_ti_c', 'noise.normalize_engine.rho_ti_c')
                    problem.model.connect('engine.c_ti_c', 'noise.normalize_engine.c_ti_c')
            if settings.fan_inlet or settings.fan_discharge:
                problem.model.connect('engine.mdot_f', 'noise.normalize_engine.mdot_f')
                problem.model.connect('engine.N_f', 'noise.normalize_engine.N_f')
                problem.model.connect('engine.DTt_f', 'noise.normalize_engine.DTt_f')
                problem.model.connect('engine.A_f', 'noise.normalize_engine.A_f')
                problem.model.connect('engine.d_f', 'noise.normalize_engine.d_f')

            problem.model.connect('trajectory.x', 'noise.geometry.x')
            problem.model.connect('trajectory.y', 'noise.geometry.y')
            problem.model.connect('trajectory.z', 'noise.geometry.z')
            problem.model.connect('trajectory.alpha', 'noise.geometry.alpha')
            problem.model.connect('trajectory.gamma', 'noise.geometry.gamma')
            problem.model.connect('trajectory.c_0', 'noise.geometry.c_0')
            problem.model.connect('trajectory.T_0', 'noise.geometry.T_0')
            problem.model.connect('trajectory.t_s', 'noise.geometry.t_s')
            
            problem.model.connect('trajectory.TS', 'noise.source.TS')
            problem.model.connect('trajectory.M_0', 'noise.source.M_0')
            problem.model.connect('trajectory.c_0', 'noise.source.c_0')
            problem.model.connect('trajectory.rho_0', 'noise.source.rho_0')
            problem.model.connect('trajectory.mu_0', 'noise.source.mu_0')
            problem.model.connect('trajectory.T_0', 'noise.source.T_0')
            
            if settings.airframe:
                problem.model.connect('trajectory.I_landing_gear', 'noise.source.I_landing_gear')
                problem.model.connect('trajectory.theta_flaps', 'noise.source.theta_flaps')

            problem.model.connect('trajectory.x', 'noise.propagation.x')
            problem.model.connect('trajectory.z', 'noise.propagation.z')
            problem.model.connect('trajectory.rho_0', 'noise.propagation.rho_0')
            problem.model.connect('trajectory.I_0', 'noise.propagation.I_0')
            
            problem.model.connect('trajectory.c_0', 'noise.levels.c_0')
            problem.model.connect('trajectory.rho_0', 'noise.levels.rho_0')
        ...
        if objective == 'noise':
            problem.model.add_constraint('noise.lateral', ref=1., upper=settings.noise_constraint_lateral, units=None)
            
            problem.model.add_objective('noise.flyover', ref=1.)
