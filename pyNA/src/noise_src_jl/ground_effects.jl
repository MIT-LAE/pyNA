function ground_effects!(spl_sb, pyna_ip, settings, f_sb, x_obs, r, beta, c_bar, rho_0)
    ...
    eta = 2 * π * rho_0 * f_sb / settings["ground_resistance"]
    ...
    U = zeros(eltype(r), (settings["n_frequency_subbands"]*settings["n_frequency_bands"],))
    ...
    if dr == 0
        @. spl_sb *= (1. + R^2 + 2 * R * exp(-(settings["incoherence_constant"] * k * dr)^2) * cos(alpha + k * dr))
    else
        @. spl_sb *= (1. + R^2 + 2 * R * exp(-(settings["incoherence_constant"] * k * dr)^2) * cos(alpha + k * dr) * sin(eps * k * dr) / (eps * k * dr))
    end
end