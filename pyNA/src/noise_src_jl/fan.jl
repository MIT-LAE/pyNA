...
function fan!(spl, pyna_ip, settings, ac, f, shield, M_0, c_0, T_0, rho_0, theta, DTt_f_star, mdot_f_star, N_f_star, A_f_star, d_f_star, comp::String)
    ...
    if settings.fan_liner_suppression
        if comp == "fan_inlet"
            supp = pyna_ip.f_supp_fi.(f, theta.*ones(T, settings.N_f, ))
        elseif comp == "fan_discharge"
            supp = pyna_ip.f_supp_fd.(f, theta.*ones(T, settings.N_f, ))
        end
        msap_j = supp .* msap_j
    end
