### jaxinterp2d package by Adam Coogan (https://github.com/adam-coogan/jaxinterp2d)
### Source code retrieved on March 7, 2023
...
def interp2d(
    ...
    yp: jnp.ndarray,
    zp: jnp.ndarray,
    fill_value: Optional[jnp.ndarray] = None,) -> jnp.ndarray:
    ...
    z_xy1 = (xp[ix] - x) / (xp[ix] - xp[ix - 1]) * z_11 + (x - xp[ix - 1]) / (xp[ix] - xp[ix - 1]) * z_21
    z_xy2 = (xp[ix] - x) / (xp[ix] - xp[ix - 1]) * z_12 + (x - xp[ix - 1]) / (xp[ix] - xp[ix - 1]) * z_22

    z = (yp[iy] - y) / (yp[iy] - yp[iy - 1]) * z_xy1 + (y - yp[iy - 1]) / (yp[iy] - yp[iy - 1]) * z_xy2

    if fill_value is not None:
        oob = jnp.logical_or(x < xp[0], jnp.logical_or(x > xp[-1], jnp.logical_or(y < yp[0], y > yp[-1])))
...
class CartesianGrid:
    ...
    def __call__(self, *coords) -> jnp.ndarray:
        """
