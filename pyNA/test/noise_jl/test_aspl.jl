...
@testset "aspl module" begin
    ...
    ## Test 1: A-weighting with delta-dB=zero weights
	if true
		# Data
		...
		@test(isapprox(f_aspl(data, spl), f_oaspl(spl), rtol=1e-6))

	end

	## Test 2: A-weighting with delta-dB weights
	if true
		# Data
		l_i = 16
		f = 10 .^ (0.1 * range(1+l_i, 40, length=24))
	    ...
	    # Inputs
		spl = reshape([58.28, 57.84, 56.18, 53.52, 49.69, 44.04, 37.77, 30.59, 27.62, 27.52, 25.01, 19.17, 8.14, -1.53, -6.88, -24.86, -34.92, -57.23, -82.45, -117.32, -154.43, -180.96, -180.96, -180.96], (1, 24))

		@test(isapprox(f_aspl(data, spl), [40.7063505], rtol=1e-6))
