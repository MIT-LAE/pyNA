...
df = Matrix(DataFrame(CSV.File("../../data/isa/atmospheric_absorption.csv")))
...
pyna_ip = PynaInterpolations(f_abs)
...
# Compute
println("--- Compute ---")
@btime atmospheric_absorption_fwd!(y, x)
println(y)
...
const f_tape = JacobianTape(atmospheric_absorption_fwd!, Y, X)
