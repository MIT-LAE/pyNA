...
class TestMSAPSubbands(unittest.TestCase):
    ...
    def test_plot_subbands(self):
        ...
        if False: 
            ...
            f = compute_frequency_bands(n_frequency_bands=settings['n_frequency_bands'])
            f_sb = compute_frequency_subbands(f=f, n_frequency_subbands=settings['n_frequency_subbands'])
    ...
    def test_jax(self):
        ...
        spl_in = jnp.array([80., ])
        msap_in = 10**(spl_in/10.)
        f_prime_0 = jax.jacfwd(compute_msap_subbands, argnums=0)
