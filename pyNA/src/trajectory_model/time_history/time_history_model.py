...
class TimeHistoryModel(om.Problem):
    
    def create(self, num_nodes, vars, var_units) -> None:
        
        self.model.add_subsystem(name='trajectory', 
                                 subsys=TrajectoryData(num_nodes=num_nodes, vars=vars, var_units=var_units),
                                 promotes_outputs=vars)
    ...
    def set_initial_conditions(self, settings, data) -> None:
        ...
        self.set_val('trajectory.rho_0', data['rho_0 [kg/m**3]'])
        self.set_val('trajectory.mu_0', data['mu_0 [kg/m/s]'])
        self.set_val('trajectory.I_0', data['I_0 [kg/m**2/s]'])
        ...
        if settings['jet_mixing_source'] and not settings['jet_shock_source']:
            ...
            self.set_val('trajectory.jet_rho', data['jet_rho [kg/m**3]'])
            ...
            self.set_val('trajectory.jet_V', data['jet_V [m/s]'])
            self.set_val('trajectory.jet_A', data['jet_A [m**2]'])
            self.set_val('trajectory.jet_Tt', data['jet_Tt [K]'])
        ...
        if settings['core_source']:
            if settings['core_turbine_attenuation_method'] == "ge":
                ...
                self.set_val('trajectory.core_Tt_j', data['core_Tt_j [K]'])
                self.set_val('trajectory.core_Pt_i', data['core_Pt_i [Pa]'])
                self.set_val('trajectory.turb_rho_i', data['turb_rho_i [kg/m**3]'])
                self.set_val('trajectory.turb_c_i', data['turb_c_i [m/s]'])
                self.set_val('trajectory.turb_rho_e', data['turb_rho_e [kg/m**3]'])
